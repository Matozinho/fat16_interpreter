...
namespace interpreter {
  ...
  std::vector<Fat16::fat_dir_entry> Fat16::get_dir_entries(uint16_t first_cluster) {
    std::vector<uint16_t> cluster_chain = this->get_cluster_chain(first_cluster);
    Fat16::fat_dir_entry dir_entry;
    std::vector<Fat16::fat_dir_entry> dir_entries;

    const uint32_t sizeof_dir_entry = sizeof(Fat16::fat_dir_entry);
    uint32_t bytes_per_cluster
        = this->boot_record.bytes_per_sector * this->boot_record.sectors_per_cluster;

    for (uint16_t cluster : cluster_chain) {
      uint32_t dir_begin = this->get_cluster_addr(cluster);

      this->file->seekg(dir_begin, std::ios::beg);
      this->file->read(reinterpret_cast<char*>(&dir_entry), sizeof_dir_entry);
      uint32_t readed_bytes = sizeof(Fat16::fat_dir_entry);
  ...
  std::vector<uint16_t> Fat16::get_cluster_chain(uint16_t first_cluster) {
    ...
    while (next_cluster < 0xFFF8) {
      ...
      this->file->read(reinterpret_cast<char*>(&next_cluster), sizeof(uint16_t));
    }

    return cluster_chain;
  }

  uint32_t Fat16::get_cluster_addr(uint16_t cluster) {
  ...
  std::vector<uint8_t> Fat16::get_file(Fat16::fat_dir_entry file_entry) {
    std::vector<uint8_t> file;
    std::vector<uint16_t> cluster_chain = this->get_cluster_chain(file_entry.low_first_cluster);

    uint32_t bytes_per_cluster
        = this->boot_record.sectors_per_cluster * this->boot_record.bytes_per_sector;
    uint32_t remaining_bytes = file_entry.file_size;

    for (uint16_t cluster : cluster_chain) {
      uint32_t cluster_begin = this->get_cluster_addr(cluster);
  ...
  Fat16* Fat16::print_boot_record() {
    ...
    std::cout << std::hex << std::showbase << "sectors_per_cluster: "
              << static_cast<uint32_t>(this->boot_record.sectors_per_cluster) << std::endl;
    std::cout << std::hex << std::showbase
              << "reserved_sector_count: " << this->boot_record.reserved_sectors << std::endl;
    std::cout << std::hex << std::showbase
              << "table_count: " << static_cast<uint32_t>(this->boot_record.fat_count) << std::endl;
    std::cout << std::hex << std::showbase
              << "root_entry_count: " << this->boot_record.root_entry_count << std::endl;
    std::cout << std::hex << std::showbase
              << "total_sectors_16: " << this->boot_record.total_sectors_16 << std::endl;
    std::cout << std::hex << std::showbase
              << "media_type: " << static_cast<uint32_t>(this->boot_record.media_type) << std::endl;
    std::cout << std::hex << std::showbase
